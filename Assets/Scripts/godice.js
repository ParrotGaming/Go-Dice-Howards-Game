...
class GoDice {
  ...
  setLed(led1, led2) {
    ...
    if (adjustedLed1.length === 1)
      adjustedLed1.push(adjustedLed1[0], adjustedLed1[0]);
    if (adjustedLed2.length === 1)
      adjustedLed2.push(adjustedLed2[0], adjustedLed2[0]);

    const messageArray = [
      this.messageIdentifiers.SET_LED,
      ...adjustedLed1,
      ...adjustedLed2,
    ];
  ...
  sendMessage(messageArray) {
    ...
    return this.GoDiceService.getCharacteristic(
  ...
  getD6Value(data, startByte) {
    ...
    const coord = `${this.getAngleValue(xyzArray[0])}-${this.getAngleValue(
      xyzArray[1]
    )}-${this.getAngleValue(xyzArray[2])}`;
    switch (coord) {
      case "192-255-255":
        return 1;
      case "255-255-64":
  ...
  handleNotificationChanged(event) {
    this.parseMessage(
      event.target.value,
      event.target.service.device.id.toString()
    );
  }
  ...
  connectDeviceAndCacheCharacteristics() {
    ...
    return this.bluetoothDevice.gatt
      ...
      .then((characteristic) => {
        ...
        this.CubeCharacteristics.addEventListener(
          "characteristicvaluechanged",
          this.handleNotificationChanged
        );
  ...
  onDisconnectButtonClick() {
    if (this.CubeCharacteristics) {
      this.CubeCharacteristics.removeEventListener(
        "characteristicvaluechanged",
        this.handleNotificationChanged.bind(this)
      );
      this.CubeCharacteristics = null;
    }
    if (this.bluetoothDevice === null) return;
